...
namespace {
    ...
    void deallocate(void* memory, bool isVector) {
        ...
        if (predecessor->weight == kIsVector) {
            if (!isVector) {
        ...
        } else if (predecessor->weight == kIsScalar) {
            if (isVector) {
                cerr << "You are attempting to deallocate a block of memory that you allocated with"
                       "new using the delete[] operator. This will cause memory errors. Instead,"
        ...
        } else if (predecessor->weight == kDeallocated) {
        ...
        predecessor->weight = kDeallocated;
...
int DataPoint::theAllocationTracker = 0;

int netDataPointAllocations() {
    return DataPoint::theAllocationTracker;
}

/* Allocation operators. */
void* DataPoint::operator new(size_t numBytes) {
    theAllocationTracker++;
    return allocate(numBytes, false);
}
void DataPoint::operator delete(void* memory) {
    theAllocationTracker--;
    deallocate(memory, false);
}
void* DataPoint::operator new[](size_t numBytes) {
    theAllocationTracker++;
    return allocate(numBytes, true);
}
void DataPoint::operator delete[](void* memory) {
    theAllocationTracker--;
